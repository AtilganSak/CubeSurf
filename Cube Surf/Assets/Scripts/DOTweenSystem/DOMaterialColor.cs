...
public class DOMaterialColor : DOBase
{
    ...
    public override void DO()
    {
        if (Application.isPlaying)
        {
            ...
            sourceMaterial.DOColor(endColor, duration).SetDelay(doDelay).SetEase(ease).OnComplete(() => doComplete.Invoke());
        }
        else
        {
    ...
    public override void DORevert()
    {
        if (Application.isPlaying)
        {
            ...
            sourceMaterial.DOColor(startColor, duration).SetDelay(revertDelay).SetEase(ease).OnComplete(() => doRevertComplete.Invoke());
        }
        else
        {
    ...
    public override void DOLoop()
    {
        if (Application.isPlaying)
        {
            ...
            sourceMaterial.DOColor(startColor, duration).SetDelay(revertDelay).SetEase(ease).SetLoops(-1,loopType);
        }
        else
        {
            GetComponent<Renderer>().material.color = startColor;
