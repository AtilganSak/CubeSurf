...
public class DORotate : DOBase
{
    ...
    public override void DO()
    {
        if (Application.isPlaying)
        {
            ...
            if (!local)
                c_Transform.DORotate(endValue, duration, rotateMode).SetDelay(doDelay).SetEase(ease).OnComplete(() => doComplete.Invoke());
            else
                c_Transform.DOLocalRotate(endValue, duration, rotateMode).SetDelay(doDelay).SetEase(ease).OnComplete(() => doComplete.Invoke());
        }
        else
        {
            if (!local)
    ...
    public override void DORevert()
    {
        if (Application.isPlaying)
        {
            ...
            if (!local)
                c_Transform.DORotate(startValue, duration, rotateMode).SetDelay(revertDelay).SetEase(ease).OnComplete(() => doRevertComplete.Invoke());
            else
                c_Transform.DOLocalRotate(startValue, duration, rotateMode).SetDelay(revertDelay).SetEase(ease).OnComplete(() => doRevertComplete.Invoke());
        }
        else
        {
            if (!local)
    ...
    public override void DOLoop()
    {
        if (Application.isPlaying)
        {
            ...
            if (!local)
                c_Transform.DORotate(endValue, duration, rotateMode).SetDelay(doDelay).SetEase(ease).SetLoops(-1, loopType);
            else
                c_Transform.DOLocalRotate(endValue, duration, rotateMode).SetDelay(doDelay).SetEase(ease).SetLoops(-1, loopType);
        }
        else
        {
            if (!local)
